 ...
 * just organized like this so that we don't need 21 files of boilerplate code.
...
public class Tokens {
     ...
     * account of it not implementing the visitor pattern. However, it is not just
     * an interface, because it defines common state that is used by all of its
     * descendants.
     */
    public static abstract class BaseToken {
         ...
         *
         * [CSE 262] If you are unfamiliar with the visitor pattern, the gist of it
         * is that it is a way to avoid having big nested if statements, while
         * keeping all of the code for an operation in one place. In a bit more
         * detail, we're going to want to be able to iterate through a list of
         * tokens and print them. We might want to print them a few different ways,
         * even (text, JSON, XML, etc.). The logic for printing tokens shouldn't be
         * in the Token class. That is, we shouldn't have methods like 'printAsJSON'
         * and 'printAsXML'. But we also don't want to have an object
         * 'XmlTokenPrinter' that consists of a single method with a huge if block
         * (if this_token instanceof X else if this_token instanceof Y else ...).
         *
         * [CSE 262] The visitor pattern is a sort of double indirection (don't
         * worry, in Java it's not expensive!), where the XMLPrinter will implement
         * the ITokenVisitor interface, and then pass *itself* to the visit() method
         * of the tokens. The tokens immediately "circle back" by passing themselves
         * to a more specific method of the visitor (i.e., visitX()). The net effect
         * is that the XMLPrinter gets to keep all its code in one file, but in
         * nice, easy to understand methods.
         *
         * The only catch is that we often want visitors who produce different types
         * of results. We handle this by (1) making the interface generic, and then
         * instantiating the visitor with a specific return type, and (2) putting
         * that return type in the name of the visit function. Hence, below, we have
         * visitString().
         *
    ...
    /** A token for `let` (special form) */
    public static class Let extends BaseToken {
        /** Construct by forwarding to the Token constructor */
